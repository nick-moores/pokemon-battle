...
export async function fetchPokemon(nameOrId: string | number): Promise<BasePokemon | null> {
  ...
  const pokemon: BasePokemon = {
    ...
    shinyBackSprite: data.sprites.back_shiny ?? data.sprites.front_shiny ?? '',
    shinyAnimatedSprite: data.sprites.versions?.['generation-v']?.['black-white']?.animated?.front_shiny ?? '',
    shinyAnimatedBackSprite: data.sprites.versions?.['generation-v']?.['black-white']?.animated?.back_shiny ?? '',
